...
# LangFlix TrueNAS Deployment - Start Script (Spanish Channel)
# Usage: ./run_spanish.sh [--build|-b]
...
COMPOSE_FILE="$SCRIPT_DIR/docker-compose.spanish.yml"

echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo -e "${BLUE}🚀 LangFlix TrueNAS Deployment (Spanish Channel) - Start${NC}"
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo ""
...
if [ ! -f "$COMPOSE_FILE" ]; then
    echo -e "${RED}❌ 오류: docker-compose.spanish.yml 파일을 찾을 수 없습니다${NC}"
...
if sudo docker compose -f "$COMPOSE_FILE" ps | grep -q "Up"; then
    ...
    echo "   (Spanish Channel Deployment)"
    echo ""
    
    # 마운트 경로 확인 (이전 설정과 새 설정 불일치 체크)
    OLD_MOUNT_DETECTED=false
    if sudo docker inspect langflix-spanish-ui 2>/dev/null | grep -q "assets/youtube_token.json"; then
...
echo ""
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo -e "${BLUE}📊 컨테이너 상태 (Spanish Channel)${NC}"
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
...
# Port 8001 for Spanish channel API
if curl -s http://localhost:8001/health > /dev/null 2>&1; then
    echo -e "${GREEN}✅ Spanish API 서버 정상 작동 (Port 8001)${NC}"
else
    echo -e "${YELLOW}⚠️  Spanish API 서버 응답 없음 (아직 시작 중일 수 있음)${NC}"
fi

# YouTube 자격 증명 파일 접근 검증 (컨테이너 내부)
echo ""
echo -e "${BLUE}🔍 YouTube 자격 증명 파일 접근 검증 중...${NC}"
if sudo docker ps | grep -q "langflix-spanish-ui"; then
    # 마운트 경로 확인
    echo "   마운트 경로 확인 중..."
    MOUNT_INFO=$(sudo docker inspect langflix-spanish-ui 2>/dev/null | grep -A 3 "youtube_token.json" | grep "Source" | head -1)
    if echo "$MOUNT_INFO" | grep -q "auth/youtube_token.json"; then
        ...
        echo "      해결: ./run_spanish.sh 실행 시 옵션 1 선택"
    else
        echo -e "${YELLOW}   ⚠️  마운트 정보 확인 불가${NC}"
    fi
    
    # 컨테이너 내부에서 파일 접근 테스트
    if sudo docker exec langflix-spanish-ui test -r /app/auth/youtube_credentials.json 2>/dev/null; then
        echo -e "${GREEN}✅ youtube_credentials.json 접근 가능${NC}"
        
        # 파일 크기 확인
        CREDS_SIZE=$(sudo docker exec langflix-spanish-ui stat -c%s /app/auth/youtube_credentials.json 2>/dev/null || echo "0")
    ...
    if sudo docker exec langflix-spanish-ui test -r /app/auth/youtube_token.json 2>/dev/null; then
        echo -e "${GREEN}✅ youtube_token.json 접근 가능${NC}"
        
        # 파일 크기 확인
        TOKEN_SIZE=$(sudo docker exec langflix-spanish-ui stat -c%s /app/auth/youtube_token.json 2>/dev/null || echo "0")
        if [ "$TOKEN_SIZE" -eq 0 ]; then
            ...
            # JSON 유효성 간단 확인
            if sudo docker exec langflix-spanish-ui python3 -c "import json; json.load(open('/app/auth/youtube_token.json'))" 2>/dev/null; then
    ...
    echo ""
    echo "   컨테이너 내부 파일 목록:"
    sudo docker exec langflix-spanish-ui ls -la /app/auth/youtube_*.json 2>/dev/null || echo "   파일 없음"
    
    # End-to-End 테스트 가이드
    echo ""
    echo -e "${BLUE}════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}🧪 End-to-End 테스트 가이드 (Spanish Channel)${NC}"
    echo -e "${BLUE}════════════════════════════════════════════════${NC}"
    UI_IP=$(hostname -I | awk '{print $1}')
    echo ""
    echo -e "${GREEN}1. 웹 UI 접속:${NC}"
    echo "   http://${UI_IP}:5001"
    echo ""
    ...
    echo "   sudo docker compose -f $COMPOSE_FILE logs -f langflix-spanish-ui"
    echo ""
    echo -e "${GREEN}4. 문제 발생 시 확인:${NC}"
    echo "   - 마운트 경로: sudo docker inspect langflix-spanish-ui | grep youtube_token"
    echo "   - 파일 접근: sudo docker exec langflix-spanish-ui cat /app/auth/youtube_token.json"
    echo "   - 최근 오류: sudo docker logs langflix-spanish-ui 2>&1 | tail -50"
    echo ""
    
else
    echo -e "${YELLOW}⚠️  langflix-spanish-ui 컨테이너가 실행 중이 아닙니다${NC}"
    echo "   컨테이너 시작 후 자동으로 검증됩니다"
fi

echo ""
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo -e "${GREEN}✅ LangFlix Spanish Channel 시작 완료${NC}"
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo ""
echo -e "${GREEN}🌐 접속 URL:${NC}"
echo "   - Frontend UI: http://$(hostname -I | awk '{print $1}'):5001"
...
echo "   (사용자 정의 중지 스크립트가 아직 없습니다. docker compose down으로 중지하세요)"
echo ""
