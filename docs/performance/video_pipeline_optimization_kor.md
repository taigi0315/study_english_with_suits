...
| 0 ✅ | **TICKET-037: 파이프라인 프로파일링 계측** | 없음 | **베이스라인 측정 기능** |
...
### Phase 0 상태: ✅ 완료 (TICKET-037)

**구현된 기능:**
- 구조화된 성능 측정을 위한 `PipelineProfiler` 클래스
- 쉬운 단계 계측을 위한 `profile_stage` 컨텍스트 매니저
- 프로파일링 활성화를 위한 `--profile` CLI 플래그
- 전용 프로파일링 실행을 위한 `tools/profile_video_pipeline.py` 스크립트
- 단계별 시간 및 메타데이터가 포함된 JSON 리포트 생성
- `PROFILE_STAGE` 이벤트가 포함된 구조화 로깅
...
**리포트 형식:**
- 메타데이터: 입력 파일, 설정, 타임스탬프
- 단계: 개별 단계 시간 및 선택적 메타데이터
- 요약: 총 소요 시간, 가장 느린 단계, 평균값

**다음 단계:**
- 대표 에피소드에서 베이스라인 프로파일링 실행
- 베이스라인 리포트를 `profiles/baseline/` 디렉터리에 저장
- Phase 1/2 최적화 후 개선 효과 측정에 리포트 활용
